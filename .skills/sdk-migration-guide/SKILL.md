...
```
本 Skill 将修改你的项目文件（详见 Phase 5.0 清单）。
开始前会自动执行 git stash --include-untracked 保存当前工作。
你可以随时说"放弃接入"，一切将恢复原状。
继续使用即表示你同意以下修改。
```
...
**不得跳过任何 Phase。** 如果跳过，用户将无法正确接入 AuthMS。
...
## Phase 3：租户注册（API 自动完成，无需 Web 控制台）

### 3.1 询问注册方式

```
"选择注册方式：
  [A] 免费试用（7 天，无需邀请码）— 推荐，零门槛
  [B] 邀请码注册（永久，无试用限制）— 如果你有邀请码"
```
...
```
前置校验:
  ☑ 租户名: 基于项目名自动生成 → 调 API 检查可用性 → 不可用则自动换
  ☑ 邮箱: 开发者输入 → 格式验证
  ☑ 密码: 自动生成安全密码 或 开发者自定义
  ☑ 协议: 展示 7 天试用协议 → 必须勾选
...
```
输入邀请码 → POST 同端点（带 invitation_code 字段）
→ trial=false, 永久租户
```
...
⚠️ 开始修改代码前，请确认以下事项：
  ...
  1. 以下文件将被修改/新增（见下方清单）。
  2. 你的原始代码已通过 git stash 安全保存，随时可恢复。
  3. 你的 User 表数据、.env 原有内容、业务逻辑不会被读取或修改。
  4. 本次改动不影响 package.json 的其他依赖。
  5. 如果构建失败，改动会自动撤回。
  6. 你可以随时说"放弃接入"，一切将恢复到接入前状态。

  请确认你已阅知上述风险，同意继续。
  确认？ [同意并继续] [取消]
```

等待用户明确选择[同意并继续]后，展示文件清单：
